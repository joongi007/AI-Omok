...
# 레지듀얼 블록 생성
# 컨볼루셔널 레이어(3x3 커널 128매) -> BatchNormalization 
# -> RELU -> BatchNormalization -> Add -> RELU
def residual_block():
    def f(x):
        sc = x
        x = conv(DN_FILTERS)(x)
...
# 듀얼네트워크 생성
def dual_network(label:str="renju"):
    # 모델 생성이 완료된 경우 처리하지 않음
    if os.path.exists('./model/%sbest.h5'%label): return
    ...
    # 모델 저장
    os.makedirs('./model/', exist_ok=True)  # 폴더가 없으면 생성
    model.save('./model/%sbest.h5'%label)   # 베스트 플레이어 모델
