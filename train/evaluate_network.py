...
# 1 게임 실행
def play(next_actions, target:str="renju"):
    # 상태 생성
...
# 베스트 플레이어 교대
def update_best_player(target:str="renju"):
    copy('./model/%slatest.h5'%target, './model/%sbest.h5'%target)
    print('Change BestPlayer')


# 네트워크 평가
def evaluate_network(target:str="renju"):
    # 최신 플레이어 모델 로드
    model0 = load_model('./model/%slatest.h5'%target)

    # 베스트 플레이어 모델 로드
    model1 = load_model('./model/%sbest.h5'%target)
